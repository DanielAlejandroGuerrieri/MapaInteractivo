geocodificadorModulo = (function() {
    let geocodificador; // Geocodificador que dada una dirección 
    ...
    function usaDireccion(direccion, funcionALlamar) {
     ...
     para que se obtengan las coordenadas a partir de la dirección 
     pasada por parámetro y que llame a la función pasada por
     parámetro con los siguientes parámetros
     ...
     coordenada: la ubicación de tipo google.maps.LatLng */
        /*  geocodificador.geocode({ 'address': direccion }, function(results, status) {
              if (status == 'OK') {
                  ...
                  alert('Geocode no pudo encontrar por la siguiente razon: ' + status);
              }
          });
          */


        //Modificacion propuesta por la correxion
    ...
    // Inicializo el geocoder que obtiene las corrdenadas a partir de una
    // dirección
    // La variable dirección es igual al texto ingresado por el usuario
    // Llama a la función usaDirecciin para agregarla a los listados y
    // mostrarlo en el mapa
    function inicializar() {
        var that = this
        geocodificador = new google.maps.Geocoder()

        // cuando se presiona la tecla enter en el campo direccion,
        // se agrega la dirección y se muestra en el mapa
